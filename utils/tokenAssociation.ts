...
import { AccountId, TokenId, TokenAssociateTransaction, TransactionId } from '@hashgraph/sdk';
...
export async function checkTokenAssociation(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('Failed to fetch token associations, assuming not associated');
      return {
        isAssociated: false,
        tokenId,
      };
 ...
 */
export function buildAssociationTransaction(params: AssociateTokenParams): Uint8Array {
  const { tokenId, accountId } = params;
 ...
 */
export async function requestTokenAssociation(params: AssociateTokenParams): Promise<{
  needed: boolean;
