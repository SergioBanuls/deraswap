...
export function getActiveRouter(): ContractConfig {
    ...
    if (CUSTOM_CONTRACT.address) {
        console.log(
            '🔧 Using Custom Router:',
            CUSTOM_CONTRACT.hederaId,
            CUSTOM_CONTRACT.address
        )
    ...
    const routerType = (process.env.NEXT_PUBLIC_SWAP_ROUTER_TYPE ||
        'custom') as SwapRouterType

    switch (routerType) {
        case 'custom':
            if (!CUSTOM_CONTRACT.address) {
                console.warn(
                    'Custom router not configured, falling back to ETASwap'
                )
                return ETASWAP_CONTRACT
            }
            console.log(
                '🔧 Using Custom Router (env):',
                CUSTOM_CONTRACT.hederaId
            )
            return CUSTOM_CONTRACT
        case 'etaswap':
...
export const NETWORK_CONFIG = {
    ...
    mainnet: {
        ...
        // Use API endpoints for mainnet (secure)
        mirrorNodeUrl: '/api/mirror',
    },
}
