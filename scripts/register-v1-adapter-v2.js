/**
 * Register SaucerSwapV1_v2 adapter in Exchange contract
 * (v2 = redeployed with Path library fix)
 */
...
async function main() {
  console.log("🔧 Registrando SaucerSwapV1_v2 en Exchange...\n");
  ...
  const ADAPTER_ADDRESS = process.env.SAUCERSWAP_V1_ADAPTER;
  const AGGREGATOR_ID = "SaucerSwapV1_v2"; // NEW aggregatorId for redeployed adapter

  console.log(`📋 Configuración:`);
  console.log(`- Exchange: ${EXCHANGE_ADDRESS}`);
  console.log(`- Adapter: ${ADAPTER_ADDRESS}`);
  console.log(`- Aggregator ID: ${AGGREGATOR_ID} (nueva versión)\n`);
  ...
  try {
    ...
    console.log("📝 Ahora actualiza el frontend para mapear:");
    console.log("   'SaucerSwapV1' → 'SaucerSwapV1_v2'\n");
